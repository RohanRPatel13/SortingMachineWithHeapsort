...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        // base case: size of 1: do nothing.
        ...
        if (last >= lIndex) {
            // recursive case.
            int rIndex = lIndex + 1;
            int min; // index of the min value in two children.
            if (last < rIndex) {
                // nothing in right.
                min = lIndex;
            } else {
                // comparing left and right.
                if (order.compare(array.entry(lIndex),
                        array.entry(rIndex)) > 0) {
                    min = rIndex;
                } else {
                    min = lIndex;
                }
            }
            if (order.compare(array.entry(top), array.entry(min)) > 0) {
                // recursion.
                array.exchangeEntries(top, min);
                siftDown(array, min, last, order);
            }
        }
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        if (lIndex < array.length()) {
            // has left child.
            heapify(array, lIndex, order);
            int rIndex = lIndex + 1;
            if (rIndex < array.length()) {
                // has right child.
                heapify(array, rIndex, order);
            }
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        Array<T> out = new Array1L<>(q.length());
        for (int i = 0; 0 < q.length(); i++) {
            out.setEntry(i, q.dequeue());
        }
        /*
         * Some problems happened in the code above when copying the queue to
         * the array. Problem fixed.
         */
        q.clear(); // it should be redundant
        heapify(out, 0, order);
        return out;
    }
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        this.heapSize = 0;
        this.heap = null; // force to set a empty heap.
    }
    ...
    public final T removeFirst() {
        ...
        T out = this.heap.entry(0);
        this.heap.setEntry(0, this.heap.entry(--this.heapSize));
    ...
    public final int size() {
        ...
        int out = this.heapSize;
        if (this.insertionMode) {
            out = this.entries.length();
        }
